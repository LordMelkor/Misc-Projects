...
syms K Kp C Cp     % Define the symbolic variables
                        % e.g., C = C(t); Cp=C(t+1)
                       
g1 = Kp - (1-delta)*K - K^alfa + C;
g2 = C^(-1) - bet*Cp^(-1)*(alfa*Kp^(alfa-1) + 1 -delta);

dg1 = [diff(g1,K); diff(g1,C)];     % Derivative w/r to [K(t),C(t),Z(t)]
dg1p = [diff(g1,Kp); diff(g1,Cp)]; % Derivative w/r to [K(t+1),C(t+1),Z(t+1)]

dg2 = [diff(g2,K); diff(g2,C)];     % Derivative w/r to [K(t),C(t),Z(t)]
dg2p = [diff(g2,Kp); diff(g2,Cp)]; % Derivative w/r to [K(t+1),C(t+1),Z(t+1)]

% Evaluate derivatives at steady state
% and transform from symbolic to real matrix

dg1ss = double(subs(dg1,{K,C,Kp,Cp},{Kss,Css,Kss,Css}));
dg1pss = double(subs(dg1p,{K,C,Kp,Cp},{Kss,Css,Kss,Css}));
dg2ss = double(subs(dg2,{K,C,Kp,Cp},{Kss,Css,Kss,Css}));
dg2pss = double(subs(dg2p,{K,C,Kp,Cp},{Kss,Css,Kss,Css}));
