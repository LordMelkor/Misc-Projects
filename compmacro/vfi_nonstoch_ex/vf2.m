...
% and exploting monoticity of the policy function
%
% (c) J Marcelo Ochoa 
...
nk=1000;                              % Number of grid points for the capital
Kmin=0.8*Kss;
Kmax=1.2*Kss;
...
while checkv>=tol*(1-bet);
    ...
    for i=1:nk
        C = K(i)^alfa - K(j0:end);  % Consumption 
        C = (C>0).*C + (C<=0).*0;   % Replace C=0 if C<=0
        [v1(i,1) h1j0]= max(log(C) + bet*v0(j0:end));
        h1(i,1) = h1j0 + (j0-1);
        j0 = h1(i,1);
    end
